...
function dryRunResult(method, payload) {
  if (method.toLowerCase() === "getupdates") {
    return [];
  }

  if (method.toLowerCase() === "sendmessage") {
    return {
      message_id: 0,
      date: Math.floor(Date.now() / 1000),
      chat: {
        id: payload.chat_id,
        type: payload.chat_type || "private"
      },
      text: payload.text
    };
  }
...
export async function callTelegram(method, payload = {}, options = {}) {
  ...
  while (attempt <= retries) {
    const dryRun = options.dryRun ?? process.env.BOT_DRY_RUN === "true";
    if (dryRun) {
      return {
        ok: true,
        dry_run: true,
        method,
        result: dryRunResult(method, payload || {})
      };
    }
